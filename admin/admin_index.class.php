		...
		****************************************************************/
		...
		foreach($requirements as $reqname=>$reqrow){
			...
			$this->tpl->assign_block_vars('requirements', array(
					...
					'AVAILABLE'		=> '<span style="color:'.$color.'">'.$reqrow['installed'].'</span>',
		...
		$this->tpl->assign_vars(array(
			...
			'CIRCLE_USER_VALUE'		=> ($intTotalUsers > 0) ? sprintf("%.2F", (($intTotalUsers - ($intTotalUsers-$intActiveUsers)) / $intTotalUsers) * 100) : 0,
			'CIRCLE_MEMBER_VALUE'	=> ($total_members_ > 0) ? sprintf("%.2F", (($total_members_ - $total_members_inactive) / $total_members_) * 100) : 0,
